...
bool I2C_Probe()
{
   esp_err_t err = i2c_master_probe(bus_handle, 0x29, 1000); 

   if (err == ESP_OK)
   {
      printf("Devcice Responded\r\n");

      return true;
   }  
   else
   {
      debug("Device ERROR: %X\r\n", err);
...
void I2C_Init()
{
    ...
    i2c_device_config_t dev_config = {
        ...
        .device_address = VL53_SENSOR_ADDR,
...
{                               // Set up for read
   i2c_master_transmit(dev_handle, (uint8_t*)(v->address << 1), 1, 100);
   i2c_master_transmit(dev_handle, (uint8_t*)reg, 1, 100);

   i2c_master_transmit(dev_handle, (uint8_t*)((v->address << 1) + 1), 1, 100);
}

/*void Write (vl53l0x_t * v, uint8_t reg)
{                               // Set up for write
   uint8_t buff = {v->address, reg};

   debug("TEST: %X %X\r\n", v->address, reg);
   ESP_ERROR_CHECK(i2c_master_transmit(dev_handle, &buff, sizeof(buff), TIMEOUT));
...
{
   uint8_t buff[3] = {reg, (val >> 8), val};

   debug("TEST W16: %X %X %X\r\n", v->address, reg, val);

   i2c_master_transmit(dev_handle, buff, sizeof(buff), TIMEOUT);
}

void
vl53l0x_writeReg32Bit (vl53l0x_t * v, uint8_t reg, uint32_t val)
{
   uint8_t buff[5] = {reg, (val >> 24), (val >> 16), (val >> 8), val};

   debug("TEST W32: %X %X %X\r\n", v->address, reg, val);
...
uint8_t vl53l0x_readReg8Bit (vl53l0x_t * v, uint8_t reg)
{
   ...
   i2c_master_transmit_receive(dev_handle, &txBuff, sizeof(txBuff), buff, 1, 100);
...
uint16_t vl53l0x_readReg16Bit (vl53l0x_t * v, uint8_t reg)
{
   ...
   i2c_master_transmit_receive(dev_handle, &txBuff, sizeof(txBuff), buff, 2, 100);

   debug("TEST R16 Res: %X %X\r\n", buff[0], buff[1]);

   return (buff[0] << 8) + buff[1];
}

uint32_t vl53l0x_readReg32Bit (vl53l0x_t * v, uint8_t reg)
{
   uint8_t buff[10];
   uint8_t txBuff = reg;
   ...
   i2c_master_transmit_receive(dev_handle, &txBuff, sizeof(txBuff), buff, 4, 100);
...
vl53l0x_readMulti (vl53l0x_t * v, uint8_t reg, uint8_t * dst, uint8_t count)
{
   ...
   i2c_master_transmit_receive(dev_handle, &txBuff, sizeof(txBuff), buff, (size_t)count, 100);
}
...
{
   i2c_master_transmit(dev_handle, (uint8_t*)src, count, 100);
}
...
vl53l0x_init (vl53l0x_t * v)
{
   ...
   uint8_t value = vl53l0x_readReg8Bit (v, MSRC_CONFIG_CONTROL);
   ...
   vl53l0x_writeReg8Bit (v, MSRC_CONFIG_CONTROL, value);
